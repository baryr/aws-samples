...
Sample scenario:
```
aws s3 mb s3://bartosz-lambda-helloworld
sam build
sam package --output-template-file packaged.yaml --s3-bucket bartosz-lambda-helloworld
...
curl -XGET https://<aws-api-gateway-url>/Prod/hello/

aws cloudformation delete-stack --stack-name AWS
aws s3 rb s3://bartosz-lambda-helloworld
```
...
AWS$ curl http://localhost:3000/hello
```
